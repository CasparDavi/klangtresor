...
echo ""
echo "  KlangTresor einrichten — macOS"
echo "  =============================="
echo ""
...
if command -v node >/dev/null 2>&1; then
  ...
  if [ "$v" -ge 20 ] 2>/dev/null; then
    ...
    echo "    Neu holen: https://nodejs.org (LTS-Fassung)"
    fehlt=1
  fi
else
  echo "  ✗ Node.js fehlt."
  echo "    Holen: https://nodejs.org (LTS-Fassung) — oder: brew install node"
  fehlt=1
fi
...
if command -v ffmpeg >/dev/null 2>&1; then
  ...
  echo "  ✗ ffmpeg fehlt."
  echo "    Holen:  brew install ffmpeg"
  echo "    (Ohne Homebrew: https://ffmpeg.org/download.html)"
  fehlt=1
fi

if [ "$fehlt" -eq 1 ]; then
  echo ""
  echo "  Bitte das Fehlende nachholen und diese Datei erneut doppelklicken."
  echo ""
...
npm install --no-fund --no-audit || { echo "  npm install ist gescheitert."; read -r -p "  [Eingabetaste] " _; exit 1; }
...
if [ -z "$handle" ]; then
  echo "  Ohne Alias geht es nicht weiter. Später erneut versuchen."
  read -r -p "  [Eingabetaste] " _
  exit 1
fi

echo ""
echo "  → Songliste von @$handle holen …"
node bin/sammeln.js "$handle" || { echo "  Sammeln gescheitert."; read -r -p "  [Eingabetaste] " _; exit 1; }
...
echo ""
echo "  Fertig. Der KlangTresor startet jetzt."
echo "  Adresse:  http://localhost:8788"
echo "  Im WLAN:  http://$(ipconfig getifaddr en0 2>/dev/null || echo '<IP-dieses-Macs>'):8788"
